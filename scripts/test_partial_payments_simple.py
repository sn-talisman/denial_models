#!/usr/bin/env python3
"""Simplified test for partial payment handling - uses system Python."""
...
# Try to import, but handle gracefully if modules aren't available
try:
    ...
    print("  pip install structlog pandas asyncpg sqlalchemy python-dotenv")
    sys.exit(1)
...
async def test_partial_payments():
    ...
    # get_repository() reads DATABASE_URL from environment automatically
    ...
    try:
        ...
        if "is_partially_paid" in df.columns:
            ...
            if len(partial_claims) > 0:
                ...
                for idx, (_, row) in enumerate(partial_claims.head(3).iterrows()):
                    print(f"\nExample {idx + 1}:")
